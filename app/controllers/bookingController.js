 ...
 * @param {object} req 
 * @param {object} res 
 * @returns return response detail booking
 */

const getDetailBooking = async (req, res) => {
  const { bookingId } = req.params;
  const { user_id } = req.user;
  const detailBookingQuery = 'SELECT * FROM booking WHERE id=$1 and user_id=$2'
  try {
    const { rows } = await dbQuery.query(detailBookingQuery, [bookingId, user_id]);
    const dbResponse = rows[0];
