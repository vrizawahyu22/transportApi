...
const {isEmpty, empty} = validations;
const {
 ...
 * @param {object} req 
 * @param {object} res 
 * @returns {object} returned detail trip
 */

const getDetailTrip = async (req, res) => {
  const { tripId } = req.params;

  const findTripById =  'SELECT * FROM trip WHERE id=$1';
  try {
    const { rows } = await dbQuery.query(findTripById, [tripId]);
    const dbResponse =  rows;
    console.log(dbResponse);
    if (!dbResponse[0]) {
      errorMessage.error = 'No trip with that id';
      return res.status(status.notfound).send(errorMessage);
    }
